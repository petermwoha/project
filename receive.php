...
<div class="layout-wrapper clearfix">
                <div class="content-layout">
                   <?php 
	 $parcelid=$_REQUEST['txtnumber']; 
	 
	 
	 $link=mysql_connect("localhost","root","") or die("Cannot Connect to the database!");
	
	 mysql_select_db("datarush",$link) or die ("Cannot select the database!");
	// $query="INSERT INTO received (parcelid) values('".$parcelid."')";
	$query="UPDATE parcels SET status='received' WHERE parcelid='".$parcelid."'";
		  if(!mysql_query($query,$link))
		  {die ("ERROR!,PARCEL EITHER RECEIVED OR INVALID");}
		  else
		 {
		  echo "Parcel received successfully!";}
	 ?>
